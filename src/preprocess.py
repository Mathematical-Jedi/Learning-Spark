...
df.top(5)
#%%
# df.toPandas().to_csv(project_directory/"data/clean_parking_data.csv", header=True)
# 看看是哪个报错
for column in df.columns:
    try:
        df.limit(10).select(column).toPandas().to_csv(project_directory/"data/clean_parking_data.csv", header=True)
    except:
        print(f'{column} is wrong')
        # https://stackoverflow.com/questions/76072664/convert-pyspark-dataframe-to-pandas-dataframe-fails-on-timestamp-column
        # 根据解决办法
        df = df.withColumn(column, date_format(column, "yyyy-MM-dd HH:mm:ss"))
...
s.where((col('in_hour') > 0) & (col('out_hour') < 0.5))\
    .show()
    # .groupby('berthage')\
    # .agg(min('in_time'))\
    # .sort(col('in_time').desc())\
    # .toPandas().to_csv('exist_in_time.csv', header=True)
